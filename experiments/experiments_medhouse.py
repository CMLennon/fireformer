...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        checkpoint = torch.load("fire_hp_state_dict.pt")
    else:
        checkpoint = None
    ...
    def plot_preds_on_labels(labels, outputs, ax=None):
        ...
        ax.set(xlabel="preds", ylabel="vals", title="Line of best fit")
    ...
    train_transform_fire = albumentations.Compose(
        [
            albumentations.CoarseDropout(
                min_holes=1, max_holes=8, max_height=3, max_width=3, p=0.1
            ),
            albumentations.ShiftScaleRotate(
                shift_limit=1, scale_limit=0.1, rotate_limit=15, p=0.15
            ),
    ...
    train_transform_hs = albumentations.Compose(
        [
            ...
            albumentations.ShiftScaleRotate(
                shift_limit=3, scale_limit=0.1, rotate_limit=15, p=0.15
            ),
    ...
    train_list = glob.glob(
        os.path.join("/home/connor/Desktop/MassCrop/OutputFolder/raster", "*.tif")
    )
    print(train_list)
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), weight_decay=weight_decay, lr=lrate
    )
    ...
    idlist = [
        "947fbfb1-ebea-452f-9ad5-e4fc38c05068",
        "d0d17250-64e5-4961-a1dc-42986547c3ff",
    ]

    item1 = SingleItemRetriever(
        pickle_file="/home/connor/Desktop/MassCrop/OutputFolder/tabular/tabular_inputs_res.pckl",
        raster_dir="/home/connor/Desktop/MassCrop/OutputFolder/raster",
        image_loader=tifffile_loader,
        outcome="CostPerAcre",
        label=idlist[0],
    )
    item2 = SingleItemRetriever(
        pickle_file="/home/connor/Desktop/MassCrop/OutputFolder/tabular/tabular_inputs_res.pckl",
        raster_dir="/home/connor/Desktop/MassCrop/OutputFolder/raster",
        image_loader=tifffile_loader,
        outcome="CostPerAcre",
    ...
    item1_hp = SingleItemRetriever(
        pickle_file="/home/connor/Desktop/MassCrop/OutputFolder/tabular/tabular_inputs_res.pckl",
        raster_dir="/home/connor/Desktop/MassCrop/OutputFolder/raster",
        image_loader=tifffile_loader,
        outcome="housingval_m_building_20",
        label=idlist[0],
    )
    item2_hp = SingleItemRetriever(
        pickle_file="/home/connor/Desktop/MassCrop/OutputFolder/tabular/tabular_inputs_res.pckl",
        raster_dir="/home/connor/Desktop/MassCrop/OutputFolder/raster",
        image_loader=tifffile_loader,
        outcome="housingval_m_building_20",
    ...
    for fold in range(1, 11, 1):
        ...
        fire_data_train_housing = FireManagement_Dataset(
            pickle_file="/home/connor/Desktop/MassCrop/OutputFolder/tabular/tabular_inputs_res.pckl",
            raster_dir="/home/connor/Desktop/MassCrop/OutputFolder/raster",
            outcome="housingval_m_building_20",
            transform=train_transform_hs,
            image_loader=tifffile_loader,
            train=True,
            log_label=False,
            fold=fold,
        ...
        fire_data_valid_housing = FireManagement_Dataset(
            pickle_file="/home/connor/Desktop/MassCrop/OutputFolder/tabular/tabular_inputs_res.pckl",
            raster_dir="/home/connor/Desktop/MassCrop/OutputFolder/raster",
            outcome="housingval_m_building_20",
            transform=None,
        ...
        train_loader_hs = torch.utils.data.DataLoader(
            fire_data_train_housing, batch_size=batch_size, shuffle=True
        )  # , pin_memory = True)

        valid_loader_hs = torch.utils.data.DataLoader(
            fire_data_valid_housing, batch_size=batch_size, shuffle=False
        )  # , pin_memory = True)
        ...
        if checkpoint is not None:
            ...
            smallest_valid_loss = checkpoint["smallest_valid_loss"]
            del checkpoint
        else:
            start_epoch = 0
        ...
        for epoch in range(num_epochs):
            ...
            dat_len = len(train_loader_hs)

            for i, dat in enumerate(train_loader_hs):
                model.train()
                ...
                data, label, tabular = mixup_data(
                    Images=data, y=label, tab=tabular, p=0.1, alpha=0.1
                )
                ...
                wandb.log(
                    {
                        f"intra-epoch loss hp: Epoch {epoch + 1}, fold {fold} Property Values": train_loss
                        / (i + 1)
                    }
                )
            ...
            with torch.no_grad():
                model.eval()
                for i, dat_v in enumerate(valid_loader_hs):
            ...
            if fold == 1:
                ...
                ax.bar(
                    tablist,
                    sal_data.cpu().numpy(),
                    color=cmap(rescale(sal_data.cpu().numpy())),
                )
                fig.canvas.draw()
                ...
                wandb.log(
                    {
                        f"Total Property Value saliency log fold {fold}, epoch {epoch + 1}": plt,
                        "epoch": epoch + 1,
                    }
                )
            ...
            if fold == 2:
                ...
                # plt.tight_layout()
                fig.suptitle(
                    "Saliency Map for Tabular and Raster Data, Holiday Farm Fire"
                )
            ...
            if smallest_valid_loss > valid_loss / len(valid_loader_hs):
                smallest_valid_loss = valid_loss / len(valid_loader_hs)
                mod_path = "model_best_epoch_medCost_fold_" + str(fold) + ".pth"
                torch.save(model, mod_path)

            print(
                f"Epoch {epoch+1} \t\t Training Loss: {epoch_loss / len(train_loader_hs)} \t\t Validation Loss: {valid_loss / len(valid_loader_hs)}"
            )
            wandb.log(
                {
                    f"valid loss, housing, fold {fold}": valid_loss
                    / len(valid_loader_hs),
                    f"train loss, housing, fold {fold}": epoch_loss
                    / len(train_loader_hs),
                    "epoch": epoch + 1,
                }
            )
        ...
        mod_path = "model_finished_training_medCost_fold" + str(fold) + ".pth"
        torch.save(model, mod_path)
        irwinidls = []
        guesses = []
        ...
        with torch.no_grad():
            model.eval()
            for i, dat in enumerate(valid_loader_hs):
                # switch to evaluation mode
            ...
            valid_out_fire.to_csv(
                "train_tabular_data_with_preds_v2_hp_fold{}.csv".format(fold)
            )
